...
void AnalyzeOrder::parse_fields(int iarg, int narg, char **arg)
{
		...
		else if (strcmp(arg[iarg], "translational") == 0) {
			method = TRANSLATIONAL;
			field_ncols[nfields] = 1; 
			addfield("translational", &AnalyzeOrder::translationalorder, SCALAR, FLOAT);
			iarg++;
		}
		else if (strcmp(arg[iarg], "translationalfcc") == 0) {
			method = TRANSLATIONAL;
			field_ncols[nfields] = 1;
			addfield("translationalfcc", &AnalyzeOrder::translationalorderfcc, SCALAR, FLOAT);
			iarg++;
		}
		else break;
	} // while (iarg < nargs)
...
void AnalyzeOrder::translationalorder()
{
	...
	double *thita, *fi;
	double ***Y;
	double rij, rijsq, rshell, rfirst, rfirstsq, r_cutsq;
	int num_shell, i_shell;
	double *dis_shell, *shell;
	double density, volume;

	int Ns = 40;
	double length_shell = 7 * r_cut;
	double width_shell = length_shell / Ns;
	dis_shell = (double *)memory->smalloc(Ns * sizeof(double), "position of each shell");
	shell = (double *)memory->smalloc(Ns * sizeof(double), "number of particles in each shell");
	order_local = (double *) memory->smalloc(nparticles * sizeof(double), "Local Translational Order");
	volume = domain->regions[rid]->volume;

												// number of shells
	r_cutsq = r_cut * r_cut;
	for (i = 0; i < Ns; i++)
		dis_shell[i] = i * length_shell / Ns;
	...
	for (i = 0; i < nlocal; i++){
		...
		for (ii = 0; ii < Ns; ii++)
			shell[ii] = 0;
		int Np = 0;
		for (j = 0; j < nlocal + particle->nghost; j++){
				jtype = type[j];
			delx = xtmp - x[j][0];
			dely = ytmp - x[j][1];
			delz = ztmp - x[j][2];
		...
		for (k = 0; k < Ns; k++){
			Nideal = Np * (pow(k + 1, 3) - pow(k, 3)) / pow(Ns, 3);
			order_local[i] = order_local[i] + fabs(shell[k] - Nideal);
		}
	...
	for (ii = 0; ii < nlocal; ii++)
		sum_local += order_local[ii];
	MPI_Allreduce(&sum_local, &sum_total, 1, MPI_DOUBLE, MPI_SUM, mworld);
	order = sum_total / nparticles;
  	memory->destroy(order_local);

}

/* ---------------------------------------------------------------------- */
/*						translational order refer FCC					  */
/* ---------------------------------------------------------------------- */
void AnalyzeOrder::translationalorderfcc()
{
	int nparticles = particle->nparticles;
	int i, j, ii, jj, k, inum, jnum, itype, jtype;
	int num_neigh;
	...
	double rij, rijsq, rshell, rfirst, rfirstsq, r_cutsq;
	int num_shell, i_shell;
	double *dis_shell, *shell, *shellfcc, *shellideal;
	double density, volume;
	...
	for (i = 0; i < nlocal; i++){
		...
		int Neighbor = 0;					//	number of local neighbors
		for (j = 0; j < nlocal + particle->nghost; j++){
			jtype = type[j];
			delx = xtmp - x[j][0];
			dely = ytmp - x[j][1];
			delz = ztmp - x[j][2];
			rijsq = (delx*delx + dely*dely + delz*delz);
			if (rijsq < 100 * r_cutsq)
				Neighbor++;
		}
		if (Neighbor == 0)
			continue;
		double density = Neighbor / (4.0 / 3.0 * PI * pow(10 * r_cut, 3));				//	local particle density
		double shell_width = (sqrt(Ns) - sqrt(Ns - 1)) * pow(4.0 / density, 1.0 / 3.0);	//	shell width 
		double r_first = 1.0 / sqrt(2) * pow(4.0 / density, 1.0 / 3.0);				//  first shell distance
		for (k = 1; k < Ns; k++)
			dis_shell[k] = r_first * sqrt((k + 1));
		for (j = 0; j < nlocal + particle->nghost; j++){
			jtype = type[j];
			delx = xtmp - x[j][0];
			dely = ytmp - x[j][1];
			...
			rijsq = (delx*delx + dely*dely + delz*delz);
			rij = sqrt(rijsq);
			if (rij <= dis_shell[Ns - 1]){
				for (k = 0; k < Ns; k++){
					if (rij > dis_shell[k] - shell_width / 2 && rij < dis_shell[k] + shell_width / 2)
						shell[k] += 1;
				}
			}
		...
		// this is the formula computing translational order 
		for (k = 0; k < Ns; k++){
			order_local[i] = order_local[i] + shell[k];
		}
		order_local[i] = fabs(order_local[i] - 97.74) / 36.26;
	}
