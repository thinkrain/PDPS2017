    ...
    for (jj = 0; jj < jnum; jj++) {
	  ...
	  if (setflag[itype][jtype]){
		  ...
		  if (rsq < cutsq[itype][jtype]){
			  ...
			  else if (lammps_flag == 1)
				  wfd = -2 * (1 - q / 2.0) * (1 - q / 2.0) * (1 - q / 2.0) ;
			  ...
			  if (visc_flag == ARTIFICIAL){
				  ...
				  mu = 4 * viscosity[itype][jtype] / (rho[i] + rho[j]) / (rsq + 0.01 * h * h) * wfd / rij_inv;
...
void PairSPH_TAITWATER::set_coeff(int narg, char **arg)
{
  ...
  if (cubic_flag == 1){
	  ...
	  a2D = 1.5915494309189533576e0 / h / h / 4.0;
	  a3D = 2.1541870227086614782e0 / h / h / h / 8.0;
  }
