			...
			for (jj = 0; jj < jnum; jj++) {
				...
				if (setflag[itype][jtype]){
					...
					if (rsq < cutsq[itype][jtype]) {
						...
						// special case to find the porosity of certain liquid
						if (inclusion_flag == 2){	
							if (mask[i] & lgroupbit){
								rho[i] += mass[jtype] * wf;
							}
							...
							if (mask[i] & sgroupbit)
								poro[j] += mass[itype] * wf / rho0[itype];
							if (mask[j] & sgroupbit)
								poro[i] += mass[jtype] * wf / rho0[jtype];
						}
						else{
...
void PairSPH_RHOSUMLOCALAV::set_style(int narg, char **arg)
{
	...
	if (strcmp(arg[5], "poro") == 0)
		poro_flag = 1;
	else if (strcmp(arg[5], "inclusion") == 0)		//  compute solid as sph density
		inclusion_flag = 1;
